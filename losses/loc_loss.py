...
def v3_loc_loss(label_xywhc, pred_xywh_raw, anchor, inf, eps, coord):
    ...
    xy_loss = tf.minimum(coord * tf.reduce_sum(resp_mask * tf.square(label_xy - pred_xy), [1,2,3,4]), inf)
    wh_loss = tf.minimum(coord * tf.reduce_sum(resp_mask * tf.square(label_wh - pred_wh), [1,2,3,4]), inf)
...
def v3_paper_loc_loss(label_xywhc, pred_xywh_raw, anchor, inf, eps, coord):
    ...
    xy_loss = tf.minimum(coord * tf.reduce_sum(resp_mask * tf.abs(label_xy - pred_xy), [1,2,3,4]), inf)
    wh_loss = tf.minimum(coord * tf.reduce_sum(resp_mask * tf.abs(label_wh - pred_wh), [1,2,3,4]), inf)
...
def v2_loc_loss(label_xywhc, pred_xywh_raw, anchor, inf, eps, coord):
    ...
    xy_loss = tf.minimum(coord * tf.reduce_sum(resp_mask * tf.square(label_xy - pred_xy), [1,2,3,4]), inf)
    wh_loss = tf.minimum(coord * tf.reduce_sum(resp_mask * tf.square(tf.math.sqrt(tf.maximum(label_wh - pred_wh, eps))), [1,2,3]), inf)
